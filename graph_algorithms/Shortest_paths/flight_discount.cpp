...
void djk(ll vertex, ll end, vector<ll> & dist, vector<ll>& vis,vector<vector<pair<ll,ll>>>adj){
    dist[vertex] = 0;
    pq.push(mp(0,vertex));
    while(!pq.empty()){
        pair<ll,ll> pop = pq.top();
        pq.pop();
        ll distance  = -1 * pop.first;
        ll popped_vertex = pop.second;

        if (vis[popped_vertex] == 1) continue; //CRUCIAL STEP  FOR MULTIPLE EDGES SAME STEP ADDITION INTO THE QUEUE

        vis[popped_vertex] = 1;
        // cout<<"setting vis ["<<popped_vertex<<"] to one "<<endl;
        dist[popped_vertex] = distance;
        // cout<<"setting distance of "<<popped_vertex<<" to "<<distance<<endl;
        for (auto x: adj[popped_vertex]){
            // cout<<"checking neighbours of "<<popped_vertex<<endl;
            if (vis[x.f] == 1){
                continue;
            }
            else{
                pq.push(mp(-1 * (dist[popped_vertex] + x.second), x.f));
                // cout<<"pushing distance of "<<x.f <<" to "<<(dist[popped_vertex] + x.second)<<endl;
            }
        }
    }
...
int main()
{
    ...
    for (auto edge : edges){
        ...
        ans = min(ans, dist[start] + dist_reversed[end] + weight/2);
    }
    cout<<ans<<endl;
}